...
export function barToBeat(barCache: TimeCache, bar: number): number {
  const [cbar, cbeat, ccpb] = barCache[partitionPoint(barCache, ([cbar]) => cbar <= bar) - 1];
  return cbeat + (bar - cbar) * ccpb;
}

export function beatToBar(barCache: TimeCache, beat: number): [bar: number, beat: number] {
  const [cbar, cbeat, ccpb] = barCache[partitionPoint(barCache, ([, cbeat]) => cbeat <= beat) - 1];
...
export function initBeatCache(barCache: TimeCache, events: any[]): TimeCache {
  ...
  for (const event of events) {
    ...
    switch (event.type) {
      case "PlaySong":
        spbs.set(barToBeat(barCache, event.bar - 1) + (event.beat - 1), 60 / event.bpm);
        break;
      case "SetBeatsPerMinute":
        spbs.set(barToBeat(barCache, event.bar - 1) + (event.beat - 1), 60 / event.beatsPerMinute);
        break;
    }
  }
  const result: TimeCache = [];
...
export function beatToTime(beatCache: TimeCache, beat: number): [time: number, spb: number] {
  const [cbeat, ctime, cspb] = beatCache[partitionPoint(beatCache, ([cbeat]) => cbeat <= beat) - 1];
  return [ctime + (beat - cbeat) * cspb, cspb];
}

export function timeToBeat(beatCache: TimeCache, time: number): number {
  const [cbeat, ctime, cspb] = beatCache[partitionPoint(beatCache, ([, ctime]) => ctime <= time) - 1];
